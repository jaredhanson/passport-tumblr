...
/**
 * Authenticate request by delegating to Tumblr using OAuth.
 *
 * @param {Object} req
 * @api protected
 */
Strategy.prototype.authenticate = function (req, options) {
  // Tumblr doesn't callback with any error parameter when users decline authorization
  // but there is an `auth_token` after a success. (We first check the session, to see if we're after a callback.)
  if (req.session[this._key] && !(req.query && req.query.auth_token)) return this.fail();
  OAuthStrategy.prototype.call(this, req, options);
}
